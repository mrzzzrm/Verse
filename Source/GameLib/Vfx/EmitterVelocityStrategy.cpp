...
EmitterConeStrategy::EmitterConeStrategy(
    float angle, float minSpeed, float maxSpeed)
    : m_angle(angle)
    , m_minSpeed(minSpeed)
    , m_maxSpeed(maxSpeed)
    , m_dist(0.0f, 1.0f)
...
glm::vec3 EmitterConeStrategy::generateVelocity() const
{
    ...
    const auto speed =
        m_minSpeed + m_dist(m_engine) * (m_maxSpeed - m_minSpeed);
...
EmitterAnyDirection::EmitterAnyDirection(float minSpeed, float maxSpeed)
    : m_minSpeed(minSpeed), m_maxSpeed(maxSpeed)
{
}
