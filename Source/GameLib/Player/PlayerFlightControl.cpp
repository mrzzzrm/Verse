    ...
    const UpdateFrame & updateFrame)
{
    ...
    auto targetLocalLinearVelocity = glm::vec3(
        {m_linearThrust.x * config.horizontal.maxSpeed,
         ...
         m_linearThrust.z * (m_linearThrust.z > 0 ? config.backward.maxSpeed
                                                  : config.forward.maxSpeed)});

    auto deltaLocalLinearVelocity =
        targetLocalLinearVelocity - localLinearVelocity;

    glm::vec3 localLinearAcceleration;
    if (std::fabs(deltaLocalLinearVelocity.x) >
        config.horizontal.acceleration * seconds)
    {
        localLinearAcceleration.x = config.horizontal.acceleration * seconds *
                                    SigNum(deltaLocalLinearVelocity.x);
    }
    else
    {
    ...
    if (std::fabs(deltaLocalLinearVelocity.y) >
        config.vertical.acceleration * seconds)
    {
        localLinearAcceleration.y = config.vertical.acceleration * seconds *
                                    SigNum(deltaLocalLinearVelocity.y);
    }
    else
    {
    ...
    body.setLinearVelocity(
        body.linearVelocity() +
        body.transform().directionLocalToWorld(localLinearAcceleration));
    ...
    auto deltaLocalAngularVelocity =
        targetLocalAngularVelocity - localAngularVelocity;

    glm::vec3 localAngularAcceleration;
    if (glm::length(deltaLocalAngularVelocity) >
        config.angular.acceleration * seconds)
    {
        localAngularAcceleration = glm::normalize(deltaLocalAngularVelocity) *
                                   config.angular.acceleration * seconds;
    }
    else
    {
    ...
    body.setAngularVelocity(
        ...
        body.transform().directionLocalToWorld(localAngularAcceleration));
}