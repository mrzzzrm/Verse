...
class VfxSandbox : public VerseRuntime
{
public:
    VfxSandbox()
        : VerseRuntime(
    ...
    void onApplicationStartup() override
    {
        ...
        m_camera.setOrientation(
            glm::quat({-glm::half_pi<float>(), 0.0f, 0.0f}));

        m_world.addSystem<ResourceManager>();
        m_world.addSystem<VfxSystem>(m_camera);

        m_navigator = std::make_shared<DebugCameraNavigator3D>(
            m_camera, input(), 1000.0f);
        //        m_ground =
        //        std::make_shared<DebugGroundPlaneRenderer>(drawContext(),
        //        m_camera); m_ground->setQuadSize(10.0f);
        ...
        //        {
        //            auto lifetime =
        //            std::make_shared<EmitterRandomLifetime>(5.5f, 8.0f); auto
        //            placement =
        //            std::make_shared<EmitterGaussianSphericalPlacement>(1.0f, 1.0f);
        //            auto velocity =
        //            std::make_shared<EmitterAnyDirection>(0.2f, 0.5f); auto
        //            intensity = std::make_shared<EmitterNoisyIntensity>(30,
        //            0.0f); auto rotation =
        //            std::make_shared<EmitterViewBillboardStrategy>(); auto
        //            color =
        //            std::make_shared<EmitterColorOverLifetime>(glm::vec4{0.4f,
        //            0.4f, 0.4f, 0.4f},
        //                                                                    glm::vec4{0.4f,
        //                                                                    0.4f,
        //                                                                    0.4f,
        //                                                                    0.0f});
        //            auto size =
        //            std::make_shared<EmitterSizeOverLifetime>(3.0f, 7.0f);
        //
        ...
        for (int i = 0; i < 10; i++)
        {
            auto emitterInstance =
                std::make_shared<EmitterInstance>(m_emitterSmoke);

            auto pose = Pose3D::atOrientation(
            ...
            m_world.systemRef<VfxSystem>().manager().addEmitterInstance(
                emitterInstance);
        }
        //        for (int i = 0; i < 10; i++)
        //        {
        //            auto emitterInstance =
        //            std::make_shared<EmitterInstance>(m_emitterSmoky);
        //
        //            auto pose =
        //            Pose3D::atOrientation(glm::quat(glm::vec3{glm::half_pi<float>(),
        //            0.0f, 0.0f})); pose.setPosition({i * 10.0f, 10.0f,
        //            -20.0f});
        //
        //            emitterInstance->setBasePose(pose);
        //            emitterInstance->setTargetPose(pose);
...
int main(int argc, char * argv[]) { return VfxSandbox().run(argc, argv); }
